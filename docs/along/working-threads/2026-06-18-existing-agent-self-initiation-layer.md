...
Continue Core/MCP Minimal Server Design in design mode. The approved shape is a real but minimal, docs-backed stdio MCP server that exposes the already-approved Working Thread contract to existing agents through Resources + Tools, without adding runtime autonomy, background scheduling, presence, adapters, Memory v2, relationship modes, delegation, write delegation, MCP prompts, required LLM calls, HTTP/SSE transport, local ports, or daemon behavior. The resource boundary is approved as summary + full record: summaries support lightweight attention and overview; full records support judgment, drift classification, wrap-up drafting, and confirmed write-back. The tool boundary is approved as action-only: `classifyDrift`, `draftWrapUp`, `proposeWorkingThreadUpdate`, and `applyConfirmedWorkingThreadUpdate`. Confirmed write-back is approved for V1, but only with explicit confirmation evidence, stale proposal conflict rejection, write scope limited to Working Thread records, and section-patch-only mutation. Malformed records should be readable as partial resources with warnings, but not writable. The server must be launched with an explicit workspace root rather than inferring cwd. Code organization is approved as `src/mcp/`, leaving `src/core/` as contract-only and avoiding a new package. Implementation should use a standard MCP SDK rather than hand-rolling the protocol; dependency installation still requires explicit implementation-phase approval. The next open decision is command/script exposure for launching the stdio server.
